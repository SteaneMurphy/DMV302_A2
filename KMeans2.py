...
for i in range(2, 11):
    centroids, clusters = clustering_k_means(                           # k-means algoirthm (K=2 - K=10)
        k_value=i, 
        max_iterations=100, 
        tolerance=0.001
    )

    inertia = calculate_inertia(centroids, clusters)                    # calculate inertia
    dunn_index = calculate_dunn_index(centroids, clusters)              # calculate dunn-index
...
for i in range(len(k_values)):
    k = k_values[i]
    inertia = inertia_result[i] / 1000                                  # result scaled for report readability (1200 vs 1.2)
    dunn = dunn_result[i]
